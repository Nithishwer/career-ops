...
func NormalizeStatus(raw string) string {
	...
	switch {
	// Most restrictive first, English-only aliases.
	...
	case strings.Contains(s, "rejected"):
		return "rejected"
	case strings.Contains(s, "discarded") || strings.HasPrefix(s, "duplicate") || strings.HasPrefix(s, "dup"):
		return "discarded"
	case strings.Contains(s, "evaluated") || s == "conditional" || s == "hold" || s == "monitor" || s == "evaluate" || s == "verify":
		return "evaluated"
	default:
		return s
	}
